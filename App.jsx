...
function AdminBilling() {
  ...
  const procesarPDF = async (file) => {
    ...
    try {
      ...
      // Parsear el texto extraido del PDF de ARCA
      // Formato ARCA: DD/MM/YYYY ... $ monto al final
      const agrupado = {}
      const lineas = fullText.split('\n').filter(l => l.trim())
      
      lineas.forEach(linea => {
        const fechaMatch = linea.match(/(\d{2})\/(\d{2})\/(\d{4})/)
        // Buscar el ultimo numero con $ en la linea (Imp. Total)
        const montos = [...linea.matchAll(/\$\s*([\d.,]+)/g)]
        if (fechaMatch && montos.length > 0) {
          const ultimo = montos[montos.length - 1]
          const monto = parseFloat(ultimo[1].replace(/\./g, '').replace(',', '.'))
      ...
      if (Object.keys(agrupado).length === 0) {
        setPdfError('No se encontraron facturas en el PDF. Asegurate de subir el PDF de "Mis Comprobantes Emitidos" de ARCA.')
        return
      }
      ...
      setPdfError('Error al procesar el PDF: ' + (err.message || 'Intenta de nuevo'))
    }
  }
