...
const ADMIN_NAV = [
  ...
  { sec: 'GESTION', items: [{ lbl: 'Pagos', id: 'apay' }, { lbl: 'Calendario Fiscal', id: 'acal' }, { lbl: 'Mensajes', id: 'amsgs', badge: true }] },
]
const CLIENT_NAV = [
...
function AdminCalendario() {
  ...
  const EF = { dia: '', mes: '0', tipo: 'otro', lbl: '', col: '#1B4FD8', recurrente: true, desc: '' }
  const [form, setForm] = useState(EF)
  const F = (k, v) => setForm(p => ({ ...p, [k]: v }))
  ...
  const cellEvs = {}
  mesEvents.forEach(e => { if (!cellEvs[e.dia]) cellEvs[e.dia] = []; cellEvs[e.dia].push(e) })
  const cells = []; for (let i = 0; i < offset; i++) cells.push(null); for (let d = 1; d <= diasMes; d++) cells.push(d)

  const openNew = () => { setForm(EF); setEditEv(null); setModal(true) }
  const openEdit = (ev) => { setEditEv(ev); setForm({ ...ev, mes: String(ev.mes) }); setModal(true) }
  const save = () => {
    if (!form.dia || !form.lbl) return
    if (editEv) {
      const idx = STORE.calEvents.findIndex(e => e.id === editEv.id)
      if (idx > -1) STORE.calEvents[idx] = { ...form, dia: parseInt(form.dia), mes: parseInt(form.mes), id: editEv.id }
    } else {
      STORE.calEvents.push({ ...form, dia: parseInt(form.dia), mes: parseInt(form.mes), id: 'ce' + Date.now() })
    }
    setModal(false); forceUpd(n => n + 1)
  }
  const del = (id) => { const idx = STORE.calEvents.findIndex(e => e.id === id); if (idx > -1) STORE.calEvents.splice(idx, 1); forceUpd(n => n + 1) }
    ...
    <div className="page">
      <div className="sec-hd">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={openNew}>+ Agregar evento</button>
        </div>
      </div>
      ...
      <div className="g2" style={{ marginBottom: 16 }}>
        <div className="card">
          ...
          <div className="card-bd" style={{ paddingTop: 10 }}>
            ...
            {mesEvents.map(ev => (
              <div key={ev.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid #ECEEF4' }}>
                <div style={{ width: 10, height: 10, borderRadius: 3, background: ev.col, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 13 }}>Dia {ev.dia} - {ev.lbl}</div>
                  {ev.desc && <div style={{ fontSize: 11.5, color: '#4A5568' }}>{ev.desc}</div>}
                  <span className="badge badge-gray" style={{ fontSize: 9.5, marginTop: 2 }}>{ev.recurrente ? 'Todos los meses' : MF[ev.mes]}</span>
                </div>
                <div style={{ display: 'flex', gap: 4 }}>
                  <button className="btn btn-outline btn-sm" onClick={() => openEdit(ev)}>Editar</button>
        ...
        <div className="card">
          ...
          <div className="card-bd" style={{ paddingTop: 10, maxHeight: 300, overflowY: 'auto' }}>
            {STORE.calEvents.map(ev => (
              <div key={ev.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid #ECEEF4' }}>
                ...
                <button className="btn btn-outline btn-sm" style={{ fontSize: 10 }} onClick={() => openEdit(ev)}>Editar</button>
            ...
            {cells.map((dia, i) => { const isToday = dia === today.getDate() && mes === today.getMonth() + 1 && anio === today.getFullYear(); const evs = dia ? cellEvs[dia] : null; return (
              <div key={i} className={`cal-cell${isToday ? ' cal-cell-today' : evs ? ' cal-cell-event' : ''}`}>
                {dia && <>
                  ...
                  {evs?.map((ev, j) => <div key={j} className="cal-ev" style={{ color: isToday ? 'rgba(255,255,255,.9)' : ev.col, fontSize: 8 }}>{ev.lbl.slice(0, 3).toUpperCase()}</div>)}
                </>}
              </div>
            )})}
          </div>
        ...
        <div className="overlay" onClick={() => setModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-bd">
              ...
              <div className="form-row">
                <div className="form-group"><label className="form-label">Dia del mes *</label><input className="form-input" type="number" min="1" max="31" value={form.dia} onChange={e => F('dia', e.target.value)} placeholder="20" /></div>
                <div className="form-group"><label className="form-label">Color</label><input className="form-input" type="color" value={form.col} onChange={e => F('col', e.target.value)} style={{ height: 38, padding: 4 }} /></div>
              </div>
              <div className="form-group">
                <label className="form-check" style={{ cursor: 'pointer', fontSize: 13 }}>
                  <input type="checkbox" checked={form.recurrente} onChange={e => F('recurrente', e.target.checked)} style={{ width: 14, height: 14 }} />
                  Se repite todos los meses
                </label>
              </div>
              {!form.recurrente && (
                <div className="form-group"><label className="form-label">Mes especifico</label>
                  <select className="form-input" style={{ appearance: 'none' }} value={form.mes} onChange={e => F('mes', e.target.value)}>
                    {MF.map((m, i) => <option key={i} value={i}>{m}</option>)}
                  </select>
                </div>
              )}
            ...
            <div className="modal-ft"><button className="btn btn-outline" onClick={() => setModal(false)}>Cancelar</button><button className="btn btn-primary" onClick={save}>{editEv ? 'Guardar cambios' : 'Agregar evento'}</button></div>
          </div>
        </div>
      )}
    </div>
...
// ── CALENDARIO CLIENTE (solo lectura) ──────────────────────────
function Calendario() {
  const today = new Date()
  const [mes, setMes] = useState(today.getMonth() + 1)
  const [anio, setAnio] = useState(today.getFullYear())
  const events = (STORE.calEvents || []).filter(e => e.recurrente || parseInt(e.mes) === mes - 1)
  const diasMes = new Date(anio, mes, 0).getDate()
  const primerDia = new Date(anio, mes - 1, 1).getDay()
  const offset = primerDia === 0 ? 6 : primerDia - 1
  const cellEvs = {}; events.forEach(e => { if (!cellEvs[e.dia]) cellEvs[e.dia] = []; cellEvs[e.dia].push(e) })
  const cells = []; for (let i = 0; i < offset; i++) cells.push(null); for (let d = 1; d <= diasMes; d++) cells.push(d)
  return (
    <div className="page">
      <div className="sec-hd">
      ...
      <div className="g2" style={{ marginBottom: 16 }}>
        <div className="card">
          <div className="card-hd"><span className="card-title">Vencimientos de {MF[mes - 1]}</span></div>
          <div className="card-bd" style={{ paddingTop: 10 }}>
            {events.length === 0 && <div style={{ color: '#4A5568', fontSize: 13 }}>Sin eventos este mes</div>}
            {events.map(ev => (
              <div key={ev.id} className="alert-box alert-info" style={{ marginBottom: 8, borderLeft: `3px solid ${ev.col}`, background: ev.col + '15' }}>
                <div style={{ color: ev.col }}><strong>Dia {ev.dia} - {ev.lbl}</strong><br /><span style={{ fontSize: 11.5 }}>{ev.desc}</span></div>
              </div>
            ))}
          </div>
        </div>
        <div className="card">
          ...
          <div className="card-bd" style={{ paddingTop: 10 }}>
            {[...new Map((STORE.calEvents || []).map(e => [e.col, e])).values()].map((e, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 9, fontSize: 13 }}>
                <div style={{ width: 10, height: 10, borderRadius: 3, background: e.col, flexShrink: 0 }} />{e.lbl}
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="cal-grid">
            {cells.map((dia, i) => { const isToday = dia === today.getDate() && mes === today.getMonth() + 1 && anio === today.getFullYear(); const evs = dia ? cellEvs[dia] : null; return (
              <div key={i} className={`cal-cell${isToday ? ' cal-cell-today' : evs ? ' cal-cell-event' : ''}`}>
                {dia && <>
                  <div className="cal-day" style={isToday ? { color: '#fff', fontWeight: 700 } : {}}>{dia}</div>
                  {evs?.map((ev, j) => <div key={j} className="cal-ev" style={{ color: isToday ? 'rgba(255,255,255,.9)' : ev.col }}>{ev.lbl.slice(0, 3).toUpperCase()}</div>)}
                </>}
              </div>
            )})}
          </div>
...
function ImportarFacturas({ user }) {
  ...
  const facturas = STORE.inv[user.id] || []
    ...
    reader.onload = (e) => {
      try {
        ...
        lineas.forEach((linea, idx) => {
          if (idx === 0) return // saltar header
          const cols = linea.split(',').map(c => c.trim().replace(/"/g, ''))
          if (cols.length < 3) { errores++; return }

          // Formatos comunes de ARCA: Fecha, Tipo, Numero, Monto
          const fecha = cols[0]
          const monto = parseFloat(cols[cols.length - 1].replace(/\./g, '').replace(',', '.'))

          if (!fecha || isNaN(monto)) { errores++; return }

          // Extraer periodo del formato DD/MM/YYYY o YYYY-MM-DD
          let periodo = ''
          if (fecha.includes('/')) {
            const parts = fecha.split('/')
            if (parts.length === 3) {
              const mes = parts[1].padStart(2, '0')
              const anioF = parts[2].length === 2 ? '20' + parts[2] : parts[2]
              periodo = `${anioF}-${mes}`
            }
          ...
          if (!periodo) { errores++; return }
          importadas.push({ periodo, monto })
        })

        // Agrupar por periodo y sumar
        const agrupado = {}
        importadas.forEach(({ periodo, monto }) => {
          agrupado[periodo] = (agrupado[periodo] || 0) + monto
        })

        // Guardar en STORE
        let nuevas = 0; let actualizadas = 0
        Object.entries(agrupado).forEach(([per, monto]) => {
          if (!STORE.inv[user.id]) STORE.inv[user.id] = []
          const idx = STORE.inv[user.id].findIndex(i => i.per === per)
          if (idx > -1) { STORE.inv[user.id][idx].monto = monto; actualizadas++ }
          else { STORE.inv[user.id].push({ id: 'i' + Date.now() + Math.random(), per, monto }); nuevas++ }
        })

        setResultado({ nuevas, actualizadas, errores, total: importadas.length })
        forceUpd(n => n + 1)
      } catch (err) {
        setError('No se pudo leer el archivo. Asegurate de exportarlo como CSV desde ARCA.')
      }
      setLoading(false)
    }
  ...
  return (
    <div className="page">
      ...
      <p style={{ color: '#4A5568', fontSize: 13, marginBottom: 20 }}>Exporta tus facturas desde ARCA en formato CSV y subelas aca. El sistema las importa automaticamente a tu historial de facturacion.</p>

      {/* Instrucciones */}
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="card-hd"><span className="card-title">Como exportar desde ARCA</span></div>
        <div className="card-bd" style={{ paddingTop: 12 }}>
          {[
            { n: '1', txt: 'Entra a arca.gob.ar con tu CUIT y clave fiscal' },
            { n: '2', txt: 'Ve a "Mis Comprobantes" → "Emitidos"' },
            { n: '3', txt: 'Filtra por el periodo que queres importar' },
            { n: '4', txt: 'Hace clic en "Exportar" y elegis formato CSV o Excel' },
            { n: '5', txt: 'Guarda el archivo y subilo aca abajo' },
          ].map(s => (
            <div key={s.n} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#1B4FD8', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{s.n}</div>
              <div style={{ fontSize: 13, paddingTop: 3 }}>{s.txt}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div
        ...
        style={{ border: '2px dashed #DDE1EC', borderRadius: 12, padding: 40, textAlign: 'center', cursor: 'pointer', background: '#F7F8FC', marginBottom: 16, transition: 'all .15s' }}
        onMouseEnter={e => { e.currentTarget.style.borderColor = '#1B4FD8'; e.currentTarget.style.background = '#EEF3FF' }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = '#DDE1EC'; e.currentTarget.style.background = '#F7F8FC' }}
      >
        <input ref={fileRef} type="file" accept=".csv,.txt,.xls,.xlsx" style={{ display: 'none' }} onChange={onFileChange} />
        <div style={{ fontSize: 32, marginBottom: 10 }}>📂</div>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 6 }}>Arrastra el archivo aca o hace clic para seleccionar</div>
        <div style={{ fontSize: 12.5, color: '#4A5568' }}>Formatos aceptados: CSV, TXT, XLS, XLSX</div>
        {loading && <div style={{ marginTop: 12, color: '#1B4FD8', fontWeight: 600 }}>Procesando archivo...</div>}
      </div>

      {error && <div className="alert-box alert-danger" style={{ marginBottom: 16 }}>{error}</div>}

      {resultado && (
        <div className="alert-box alert-success" style={{ marginBottom: 16, padding: '14px 16px' }}>
          <div>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>Importacion completada</div>
            <div style={{ fontSize: 12.5 }}>{resultado.nuevas} periodos nuevos importados · {resultado.actualizadas} periodos actualizados · {resultado.errores > 0 ? `${resultado.errores} lineas con error` : 'Sin errores'}</div>
          </div>
        </div>
      )}
      ...
      {/* Historial actual */}
      <div className="card">
        <div className="card-hd"><span className="card-title">Facturacion registrada</span></div>
        <div className="tbl-wrap">
          <table>
            <thead><tr><th>Periodo</th><th>Monto</th></tr></thead>
            <tbody>
              {facturas.length === 0
                ? <tr><td colSpan={2}><div className="empty"><div className="empty-title">Sin facturacion registrada</div></div></td></tr>
                : [...facturas].sort((a, b) => b.per.localeCompare(a.per)).map(f => (
                  <tr key={f.id}><td className="fw6">{MF[parseInt(f.per.split('-')[1]) - 1]} {f.per.split('-')[0]}</td><td className="fw6">{fmt(f.monto)}</td></tr>
                ))
              }
            </tbody>
          </table>
