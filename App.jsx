...
function ClientBill({ user }) {
  ...
  return (
    <div className="page">
      ...
      <div className="card"><div className="card-hd"><span className="card-title">Detalle mensual</span></div><div className="tbl-wrap"><table><thead><tr><th>Mes</th><th>Periodo</th><th>Monto</th><th>Acumulado</th></tr></thead>
        <tbody>{invs.length === 0 ? <tr><td colSpan={4}><div className="empty"><div className="empty-title">Sin datos</div><p style={{ fontSize: 12 }}>Tu contador carga estos datos</p></div></td></tr> :
          [...invs].sort((a, b) => b.per.localeCompare(a.per)).map(f => { const mi = parseInt(f.per.split('-')[1]) - 1; const acum = invs.filter(fi => fi.per <= f.per).reduce((s, fi) => s + fi.monto, 0); return <tr key={f.id}><td className="fw6">{MF[mi]}</td><td style={{ fontSize: 12, color: '#4A5568' }}>{f.per}</td><td className="fw6">{fmt(f.monto)}</td><td style={{ color: '#4A5568' }}>{fmt(acum)}</td></tr> })
        }</tbody>
      </table></div></div>
    </div>
  )
