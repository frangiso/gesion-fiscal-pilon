  ...
  const procesarPDF = (file) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Parsear el texto del PDF de ARCA
        // Formato: fecha ... Imp. Total $  monto
        const lineas = text.split('\n').filter(l => l.trim())
        const agrupado = {}
        lineas.forEach(linea => {
          // Buscar lineas con fecha DD/MM/YYYY
          const fechaMatch = linea.match(/(\d{2})\/(\d{2})\/(\d{4})/)
          const montoMatch = linea.match(/\$\s*([\d.,]+)\s*$/)
          if (fechaMatch && montoMatch) {
            ...
            if (!isNaN(monto)) agrupado[periodo] = (agrupado[periodo] || 0) + monto
          }
        })
        if (Object.keys(agrupado).length === 0) { setPdfError('No se encontraron facturas en el PDF. Asegurate de subir el PDF correcto de ARCA.'); return }
        ...
        setPdfResult({ nuevas, actualizadas, periodos: Object.keys(agrupado) })
        await load()
      } catch (err) { setPdfError('Error al procesar el PDF: ' + err.message) }
    }
